            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="form d-flex flex-column flex-lg-row">
                ...
                <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-12">
                    <ul class="nav nav-custom nav-tabs nav-line-tabs nav-line-tabs-2x border-0 fs-4 fw-semibold mb-n2">
                        ...
                        <li class="nav-item" id="detailsTab" style="display: {{ old('is_mobile', $product->is_mobile) ? 'block' : 'none' }};">
                    ...
                    <div class="tab-content">
                                ...
                                <div class="card card-flush py-4">
                                    <div class="card-header mb-5">
                                        ...
                                        <div class="card-title">
                                            ...
                                            <input type="checkbox" class="form-check-input me-2" id="is_mobile" name="is_mobile" value="1"
                                                {{ $product->mobileDetail ? 'checked' : '' }}>
                                    ...
                                    <div class="card-body row pt-0">
                                        ...
                                        <div class="mb-10 col-md-3">
                                            ...
                                            <input type="text" class="form-control mb-2" id="code" name="code" value="{{ old('code', $product->code) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-5">
                                            <label class="form-label">{{ __('Barcode') }}</label>
                                            <div class="input-group d-flex align-items-center">
                                                <input type="text" class="form-control mb-2" style="border-top-right-radius: 0; border-bottom-right-radius: 0;" id="barcode" name="barcode" value="{{ old('code', $product->barcode) }}" required>
                                        ...
                                        <div class="mb-10 col-md-6">
                                            ...
                                            <input type="number" class="form-control mb-2" id="quantity" name="quantity" value="{{ old('quantity', $product->quantity) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-6">
                                            <label class="form-label">{{ __('Stock Alert') }}</label>
                                            <input type="number" class="form-control mb-2" id="stock_alert" name="stock_alert" value="{{ old('stock_alert', $product->stock_alert) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-3">
                                            <label class="form-label">{{ __('Cost') }}</label>
                                            <input type="number" class="form-control mb-2" id="cost" name="cost" step="0.01" value="{{ old('cost', $product->cost) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-3">
                                            <label class="form-label">{{ __('Price') }}</label>
                                            <input type="number" class="form-control mb-2" id="price" name="price" step="0.01" value="{{ old('price', $product->price) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-3">
                                            <label class="form-label">{{ __('Wholesale Price') }}</label>
                                            <input type="number" class="form-control mb-2" id="wholesale_price" name="wholesale_price" step="0.01" value="{{ old('wholesale_price', $product->wholesale_price) }}" required>
                                        </div>
                                        <div class="mb-10 col-md-3">
                                            <label class="form-label">{{ __('Lowest Price For Sale') }}</label>
                                            <input type="number" class="form-control mb-2" id="min_sale_price" name="min_sale_price" step="0.01" value="{{ old('min_sale_price', $product->min_sale_price) }}" required>
                                        </div>
                                        ...
                                        <div class="mb-10 col-md-4">
                                            ...
                                            @if($product->image)
                                                <img src="{{ asset('images/products/' . $product->image) }}" alt="{{ $product->name }}" class="img-thumbnail mb-2" style="max-width: 200px;">
                                            @endif
                        ...
                        <div class="tab-pane fade" id="kt_ecommerce_add_product_details" role="tab-panel" style="display: {{ old('is_mobile', $product->is_mobile) ? 'block' : 'none' }};">
                                ...
                                <div class="card card-flush py-4">
                                    ...
                                    <div class="card-body row pt-0">
                                        ...
                                        <div class="mb-10 col-md-3">
                                            ...
                                            <input type="number" class="form-control mb-2" id="battery_health" name="battery_health" value="{{ old('battery_health', $product->mobileDetail->battery_health ?? '') }}" min="0" max="100">
                                        </div>
                                        <div class="mb-10 col-md-3">
                                        ...
                                        <div class="mb-10 col-md-3">
                                            ...
                                            <select class="form-select mb-2" id="has_box" name="has_box">
                                                ...
                                                <option value="1" {{ old('has_box', $product->mobileDetail->has_box ?? false) ? 'selected' : '' }}>{{ __('Yes') }}</option>
                                                <option value="0" {{ old('has_box', $product->mobileDetail->has_box ?? false) == 0 ? 'selected' : '' }}>{{ __('No') }}</option>
                                            </select>
                                        </div>
                                        <div class="mb-10 col-md-12">
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <button type="submit" class="btn btn-success me-3">{{ __('Save Changes') }}</button>
    ...
    @section('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                var hasMobileDetail = {{ $product->mobileDetail ? 'true' : 'false' }};
                var detailsTab = document.getElementById('detailsTab');
                var detailsContent = document.getElementById('kt_ecommerce_add_product_details');

                detailsTab.style.display = hasMobileDetail ? 'block' : 'none';
                detailsContent.style.display = hasMobileDetail ? 'block' : 'none';

                document.getElementById('is_mobile').addEventListener('change', function() {
                    var checked = this.checked;
                    detailsTab.style.display = checked ? 'block' : 'none';
