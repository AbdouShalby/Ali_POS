    ...
    <div class="container">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="cost" name="cost" value="{{ old('cost', $product->cost) }}" step="0.01" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="wholesale_price" name="wholesale_price" value="{{ old('wholesale_price', $product->wholesale_price) }}" step="0.01" required>
            </div>

            <!-- أقل سعر للبيع -->
            <div class="mb-3">
                <label for="min_sale_price" class="form-label">أقل سعر للبيع</label>
                <input type="number" class="form-control" id="min_sale_price" name="min_sale_price" value="{{ old('min_sale_price', $product->min_sale_price) }}" step="0.01" required>
            </div>
            ...
            <div id="mobile_details" style="display: {{ old('is_mobile', $product->mobileDetail ? true : false) ? 'block' : 'none' }};">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="color" name="color" value="{{ old('color', $product->mobileDetail->color ?? '') }}">
                </div>

                <!-- المساحة -->
                <div class="mb-3">
                    <label for="storage" class="form-label">المساحة (جيجابايت/تيرابايت)</label>
                    <input type="text" class="form-control" id="storage" name="storage" value="{{ old('storage', $product->mobileDetail->storage ?? '') }}">
                </div>

                <!-- صحة البطارية -->
                <div class="mb-3">
                    <label for="battery_health" class="form-label">صحة البطارية (%)</label>
                    <input type="number" class="form-control" id="battery_health" name="battery_health" value="{{ old('battery_health', $product->mobileDetail->battery_health ?? '') }}" step="0.01">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="condition" name="condition" value="{{ old('condition', $product->mobileDetail->condition ?? '') }}">
                </div>

                <!-- وصف الجهاز -->
                <div class="mb-3">
                    <label for="device_description" class="form-label">وصف الجهاز</label>
                    <textarea class="form-control" id="device_description" name="device_description" rows="3">{{ old('device_description', $product->mobileDetail->device_description ?? '') }}</textarea>
                </div>
